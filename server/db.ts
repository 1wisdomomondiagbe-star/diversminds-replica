...
export async function getIntakeSubmissions(limit = 50, offset = 0) {
  const db = await getDb();
  if (!db) {
    throw new Error("Database not available");
  }

  return db.select().from(intakeSubmissions).limit(limit).offset(offset);
}
...
export async function updateIntakeSubmissionStatus(id: number, status: string) {
  const db = await getDb();
  if (!db) {
    throw new Error("Database not available");
  }

  return db.update(intakeSubmissions).set({ status: status as any }).where(eq(intakeSubmissions.id, id));
}
